...
function normalizePhone(v){return String(v||'').replace(/[^0-9]/g,'').slice(-10)}
function toast(text){const el=document.createElement('div');el.className='wk-contact-toast';el.textContent=text;document.body.appendChild(el);setTimeout(()=>el.remove(),2800)}
...
async function searchUsers(q){
  ...
  const {data,error}=await supabase.from('profiles').select('id,username,display_name,avatar_url,xp,level,phone').or(`username.ilike.%${needle}%,display_name.ilike.%${needle}%`).limit(30);
...
async function allUsers(){
  ...
  const {data,error}=await supabase.from('profiles').select('id,username,display_name,avatar_url,xp,level,phone').neq('id',me?.id||'').order('display_name').limit(50);
  if(error){toast(error.message);return[]}
  return data||[];
}
async function createConversation(other){
  ...
  if(ids.length){
    ...
    const existing=(members||[]).filter(x=>x.user_id===other.id).map(x=>x.conversation_id);
    if(existing.length){location.reload();return}
  }
...
async function shareInvite(name=''){
  const url=`${location.origin}/?invite=winkee`;
  const text=name?`${name}, Winkee'ye katılalım! 💜`:'Winkee’ye katıl! 💜 Mesajlaş, oyna, eğlen.';
  try{
    if(navigator.share){await navigator.share({title:'Winkee',text,url});return}
    await navigator.clipboard.writeText(`${text} ${url}`);toast('Davet bağlantısı kopyalandı.');
  }catch(e){if(e?.name!=='AbortError')toast('Paylaşım açılamadı.')}
}
async function importPhoneContacts(){
  if(!('contacts' in navigator)||!navigator.contacts?.select){
    toast('Bu masaüstü tarayıcısı rehber erişimini desteklemiyor. Telefonda Chrome ile açınca rehber seçilebilir.');
    registered=await allUsers();render();
    return;
  }
  try{
    ...
    let matches=[];
    if(phones.length&&supabaseConfigured){
      const {data,error}=await supabase.rpc('find_winkee_contacts',{phone_numbers:phones});
      if(error)toast(error.message);else matches=data||[];
    }
    registered=matches.map(p=>({...p,_phone10:normalizePhone(p.phone)}));
    render();
...
async function openSheet(){
  if(backdrop)return;
  backdrop=document.createElement('div');backdrop.className='wk-contact-backdrop';
  backdrop.innerHTML=`<section class="wk-contact-sheet" role="dialog" aria-modal="true"><div class="wk-contact-head"><div><h2>Yeni sohbet</h2><div class="wk-contact-note">Kullanıcı adıyla direkt ara veya rehberini bağla. Kayıtlı olmayan kişilere de tek dokunuşla davet gönder.</div></div><button class="wk-contact-close" aria-label="Kapat">×</button></div><div class="wk-contact-search"><span>⌕</span><input autocomplete="off" placeholder="Kişi bul · @kullanıcı adı veya isim"/></div><div class="wk-contact-actions"><button class="wk-contact-action" data-contacts><strong>📱 Rehberden kişileri bul</strong><small>Kayıtlıysa Mesaj, kayıtlı değilse Davet et</small></button><button class="wk-contact-action" data-invite><strong>💜 Winkee’ye davet et</strong><small>WhatsApp, Mesajlar ve diğer uygulamalarda paylaş</small></button></div><div class="wk-contact-hint" data-device-hint>Rehber eşleşmesi için telefondaki tarayıcıda rehber izni gerekir. Masaüstünde ise tüm Winkee üyelerini aşağıdan bulabilirsin.</div><div class="wk-contact-list"></div></section>`;
  document.body.appendChild(backdrop);
  backdrop.querySelector('.wk-contact-close').onclick=close;
  backdrop.onclick=e=>{if(e.target===backdrop)close()};
  backdrop.querySelector('[data-contacts]').onclick=importPhoneContacts;
  backdrop.querySelector('[data-invite]').onclick=()=>shareInvite();
  const input=backdrop.querySelector('input');
  input.oninput=()=>{query=input.value;clearTimeout(input._t);input._t=setTimeout(async()=>{registered=query.trim()?await searchUsers(query):await allUsers();render()},220)};
  registered=await allUsers();
  render();
  setTimeout(()=>input.focus(),60);
}
function render(){
  const list=backdrop?.querySelector('.wk-contact-list');if(!list)return;
  if(query.trim()){
    if(!registered.length){list.innerHTML='<div class="wk-contact-empty">Bu kullanıcı adı veya isimle Winkee üyesi bulunamadı.</div>';return}
    list.innerHTML=registered.map(p=>row(p)).join('');bindRows();return;
  }
  if(contacts.length){
    const byPhone=new Map(registered.filter(p=>p._phone10).map(p=>[p._phone10,p]));
    list.innerHTML=contacts.map(c=>{
      const p=(c.tel||[]).map(normalizePhone).map(n=>byPhone.get(n)).find(Boolean);
      return p?row(p):`<div class="wk-contact-row"><div class="wk-contact-avatar">👤</div><div class="wk-contact-copy"><strong>${esc((c.name||[]).join(' ')||'Kişi')}</strong><span>${esc((c.tel||[])[0]||'')}</span></div><button class="wk-contact-btn invite" data-invite-contact="${esc((c.name||[]).join(' '))}">Davet et</button></div>`;
    }).join('')||'<div class="wk-contact-empty">Rehberinde kişi bulunamadı.</div>';
    bindRows();return;
  }
  list.innerHTML=registered.length?registered.map(p=>row(p)).join(''):'<div class="wk-contact-empty">Henüz başka Winkee kullanıcısı yok. Kullanıcı adını biliyorsan yukarıdan ara.</div>';
  bindRows();
}
...
function bindRows(){
  backdrop?.querySelectorAll('[data-user]').forEach(b=>b.onclick=()=>{const p=registered.find(x=>x.id===b.dataset.user);if(p)createConversation(p)});
  backdrop?.querySelectorAll('[data-invite-contact]').forEach(b=>b.onclick=()=>shareInvite(b.dataset.inviteContact));
}
function hook(){
