...
.wk-profile-phone-wrap{margin-top:10px}
.wk-profile-phone-label{display:block;color:#a79caf;font-size:13px;margin:0 0 7px}
.wk-profile-phone{width:100%;box-sizing:border-box;background:#0e0914;border:1px solid rgba(255,255,255,.08);color:#fff;border-radius:14px;padding:14px;outline:0;font:inherit}
.wk-profile-phone:focus{border-color:#a632ff;box-shadow:0 0 0 3px rgba(166,50,255,.12)}
.wk-birth-label{display:block;color:#a79caf;font-size:13px;margin:0 0 7px}
.wk-profile-saving{opacity:.7;pointer-events:none}
`;
...
async function getProfile(){
  ...
  if(!data){
    ...
    const username=String(m.username||user.email?.split('@')[0]||'kullanici').toLowerCase().replace(/[^a-z0-9_]/g,'').slice(0,24).padEnd(3,'0');
...
function addBirthLabel(){
  document.querySelectorAll('.auth-field input[type="date"]').forEach(input=>{
    input.setAttribute('aria-label','Doğum tarihi');
    input.setAttribute('title','Doğum tarihi');
    const field=input.closest('.auth-field');
    if(field&&!field.querySelector('.wk-birth-label')){
      const label=document.createElement('span');label.className='wk-birth-label';label.textContent='Doğum tarihi';
...
function findEditModal(){
  return [...document.querySelectorAll('.modal-card')].find(x=>/Profilini düzenle/i.test(x.textContent||''));
}
...
function hydrateEditModal(profile){
  ...
  if(inputs.length>=2){
    setInputValue(inputs[0],profile.display_name||profile.username||'');
    setInputValue(inputs[1],profile.username||'');
    inputs[0].placeholder='Adın';
    inputs[1].placeholder='@kullanıcı adı';
    inputs[1].setAttribute('autocomplete','username');
  }
  if(textareas[0]){
    setInputValue(textareas[0],profile.bio||'');
    textareas[0].placeholder='Kendinden bahset...';
  }
  ...
  const phone=phoneWrap.querySelector('input');
...
async function saveEdit(){
  const modal=findEditModal();
  if(!modal||!supabaseConfigured)return false;
  ...
  const save=modal.querySelector('button:not([aria-label])');
  ...
  try{
    ...
    if(data?.username)await supabase.auth.updateUser({data:{username:data.username,display_name:data.display_name,birth_date:data.birth_date}});
    modal.querySelector('.modal')?.remove?.();
    document.dispatchEvent(new CustomEvent('wk-profile-saved'));
    location.reload();
...
async function hydrate(){
  ...
  hydrateEditModal(profile);
}

function start(){
  hookSave();
  observer=new MutationObserver(()=>{clearTimeout(start._t);start._t=setTimeout(hydrate,40)});
  observer.observe(document.body,{subtree:true,childList:true});
  hydrate();
  if(supabaseConfigured){supabase.auth.onAuthStateChange(()=>{cachedUser=null;cachedProfile=null;setTimeout(hydrate,100)});}
}
