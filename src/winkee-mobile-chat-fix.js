...
.conversation{height:100dvh;min-height:0;max-height:100dvh;padding:0 0 calc(76px + env(safe-area-inset-bottom));overflow:hidden;display:flex;flex-direction:column;}
.chat-header{flex:0 0 76px;}
...
.bottom-nav{height:76px;padding-bottom:max(7px,env(safe-area-inset-bottom));z-index:100;}
@media (max-width:600px){.conversation{padding-bottom:calc(76px + env(safe-area-inset-bottom));}.chat-header{padding-left:10px;padding-right:10px;}.messages{padding-left:12px;padding-right:12px;}.composer-tools{padding-left:12px;padding-right:12px;}.composer{margin-left:12px;margin-right:12px;}}
`;
