...
.conversation{height:100dvh!important;min-height:0!important;max-height:100dvh!important;margin:0!important;padding:0 0 calc(76px + env(safe-area-inset-bottom))!important;overflow:hidden;display:flex;flex-direction:column;}
...
@media (max-width:600px){
  ...
  .composer-tools{padding-left:12px;padding-right:12px;}
...
function reactInputValue(input,value){
  if(!input)return;
...
async function findCurrentConversation(userId){
  const title=document.querySelector('.conversation .chat-title strong')?.textContent?.trim();
  if(!title)return null;
  const {data:mine,error:mineError}=await supabase.from('conversation_members').select('conversation_id').eq('user_id',userId);
  if(mineError||!mine?.length)return null;
  const ids=mine.map(x=>x.conversation_id);
  const {data:members,error:membersError}=await supabase.from('conversation_members').select('conversation_id,user_id').in('conversation_id',ids);
  if(membersError)return null;
  const otherIds=[...new Set((members||[]).filter(x=>x.user_id!==userId).map(x=>x.user_id))];
  ...
  const wanted=(profiles||[]).find(p=>p.display_name===title||p.username===title);
  if(!wanted)return null;
  const member=(members||[]).find(x=>x.user_id===wanted.id);
  return member?.conversation_id||null;
}

function addSentBubble(body){
  const box=document.querySelector('.conversation .messages');
  if(!box)return;
  const row=document.createElement('div');
  row.className='msg mine wk-direct-message';
  row.innerHTML=`<div class="bubble"></div>`;
  const bubble=row.querySelector('.bubble');
  bubble.textContent=body;
  const meta=document.createElement('small');
  meta.textContent=new Intl.DateTimeFormat('tr-TR',{hour:'2-digit',minute:'2-digit'}).format(new Date());
...
function hookSend(){
  document.addEventListener('click',async e=>{
    ...
    if(btn.dataset.wkSending==='1')return;
    btn.dataset.wkSending='1';
    try{await sendDirect();}finally{btn.dataset.wkSending='0';}
  },true);

  document.addEventListener('keydown',async e=>{
    const input=e.target.closest?.('.conversation .composer input');
